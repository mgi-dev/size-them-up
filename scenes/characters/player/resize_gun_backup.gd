...
- line 2D
raycast
particle beam (no emiting)
no audio

position setters, color setter.
...
func manage_reflector():
	var direction = target_position.normalized()
	var normal = get_collision_normal()
	var reflected = direction.bounce(normal)
	var unknown = target_position + reflected * 5000
	var bounced_ray = RayCast2D.new()
	
	bounced_ray.position = target_position
	bounced_ray.target_position = unknown
	print("old ray start : ", position, " target: ", target_position)
	print("new ray start : ", bounced_ray.position, " target: ", bounced_ray.target_position)
	add_child(bounced_ray) 
